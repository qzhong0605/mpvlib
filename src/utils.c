...
struct timer_pool {
    ...
    uint64_t samples[MPV_PERF_SAMPLE_COUNT];
    int sample_idx;
    int sample_count;
...
void timer_pool_stop(struct timer_pool *pool)
{
    ...
    if (res) {
        ...
        pool->sample_count = MPMIN(pool->sample_count + 1, MPV_PERF_SAMPLE_COUNT);
        pool->samples[pool->sample_idx++] = res;
        pool->sample_idx %= MPV_PERF_SAMPLE_COUNT;
        ...
        if (res >= pool->peak) {
            ...
            uint64_t peak = res;
            for (int i = 0; i < MPV_PERF_SAMPLE_COUNT; i++)
...
struct mp_pass_perf timer_pool_measure(struct timer_pool *pool)
{
    ...
    int idx = pool->sample_idx - pool->sample_count + MPV_PERF_SAMPLE_COUNT;
    for (int i = 0; i < res.count; i++) {
        idx %= MPV_PERF_SAMPLE_COUNT;
